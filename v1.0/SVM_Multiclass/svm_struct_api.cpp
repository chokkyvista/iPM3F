...
//      sparm->num_classes=sample.examples[i].y.classlabel+0.1;
  ...
  if (sparm->loss_function == 2) {
	  /* specific sizePsi for bilinear SVM optimization */
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  /* Finds the label ybar for pattern x that that is responsible for
	...
	for(classlabel=1; classlabel <= sparm->num_classes; classlabel++) {
		...
		score += loss(y,ybar,sparm,x.doc->fvec);
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm, const SVECTOR *fvec)
{
	...
	if(sparm->loss_function == 1) { /* type 1 loss: squared difference */
		...
		exit(1);
		*/
		if (sparm->loss_function == 2) { /* specific loss for bilinear SVM optimization */
