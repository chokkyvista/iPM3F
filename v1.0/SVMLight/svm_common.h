...
# define FNUM    int         /* the type used for storing feature ids */
...
typedef struct svector {
  TOKEN    *words;              /* The features/values in the vector by
...
typedef struct doc {
  ...
  long    orgDocNum;           /* Document ID. Used in cutting plance method, where docnum
							      of support vector is the ID in constraint set, and 
								  orgDocNum is the ID in the training set array. */
  ...
  SVECTOR *fvec;               /* Feature vector of the example. The
				  ...
				  factor of -1. */
  int *lvec;                    /* added by Jun Zhu. The loss-augmented prediction
				  for each document. used for identify the constraints in the 
				  1-slack formulation.
								*/
} DOC;
...
SVECTOR *create_svector(TOKEN *, char *, double);
SVECTOR *create_svector_shallow(TOKEN *, char *, double);
...
int    parse_document(char *, TOKEN *, double *, long *, long *, double *, long *, long, char **, bool);
