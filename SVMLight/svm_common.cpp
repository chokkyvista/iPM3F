...
SVECTOR *create_svector(TOKEN *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (TOKEN *)my_malloc(sizeof(TOKEN)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
  vec->twonorm_sq=-1;
...
SVECTOR *create_svector_shallow(TOKEN *words,char *userdefined,double factor)
...
SVECTOR *create_svector_n_r(double *nonsparsevec, long maxfeatnum, char *userdefined, double factor, double min_non_zero)
{
  ...
  for(i=1;i<=maxfeatnum;i++)  
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero))
      fnum++;
  vec = (SVECTOR *)my_malloc(sizeof(SVECTOR));
  vec->words = (TOKEN *)my_malloc(sizeof(TOKEN)*(fnum+1));
  fnum=0;
  for(i=1;i<=maxfeatnum;i++) { 
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero)) {
	...
	SVECTOR is used */
{
    ...
    sum=(TOKEN *)my_malloc(sizeof(TOKEN)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
{
  SVECTOR *sum,*f;
  TOKEN    empty[2],*ai,*concat,*concati,*concat_read,*concat_write;
  long    length,i;
  double  weight;
  ...
  if(a){
    ...
    concat=(TOKEN *)my_malloc(sizeof(TOKEN)*(length+1));
    concati=concat;
    for(f=a;f;f=f->next) {
      ai=f->words;
      while (ai->wnum) {
    ...
    qsort(concat,length,sizeof(TOKEN),compareup_word);
		   ...
		   char **comment, bool bModel)
{
	...
	if ( !bModel ) {
		int tmpN;
		if ( sscanf(line, "%d", &tmpN) == EOF ) return 0; // read the first size of the doc.
	...
	while((pos+=read_word(line+pos,featurepair)) &&
		(featurepair[0]) && (wpos<max_words_doc)) 
	{
		/* printf("%s\n",featurepair); */
		if(sscanf(featurepair,"qid:%ld%s",&wnum,junk)==1) {
			...
			(words[wpos]).wnum = wnum;
			(words[wpos]).weight = (FVAL)weight;
			for ( int k=wpos-1; k>=0; k-- ) { // switch to make in order
				if ( (words[k]).wnum > (words[k+1]).wnum ) {
					long tmp = words[k].wnum;
					double tmpVal = words[k].weight;
					words[k].wnum = words[k+1].wnum;
					words[k].weight = words[k+1].weight;
					words[k+1].wnum = tmp;
					words[k+1].weight = tmpVal;
				} else {
					break;
				}
			}
