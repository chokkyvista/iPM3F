...
int classify (int argc, char* argv[])
{
	...
	read_input_parameters(argc,argv,testfileC,modelfileC,predictionsfileC,&sparm,
		&verbosity,&struct_verbosity);

	if(struct_verbosity>=1) {
		printf("Reading model..."); fflush(stdout);
	}
	model=read_struct_model(modelfileC,&sparm);
	if(struct_verbosity>=1) {
		fprintf(stdout, "done.\n");
	}
	...
	testsample=read_struct_examples(testfileC,&sparm);
	if(struct_verbosity>=1) {
		printf("done.\n"); fflush(stdout);
	}
	...
	if ((predfl = fopen (predictionsfileC, "w")) == NULL)
	{ perror (predictionsfileC); exit (1); }
	...
	for(i=0;i<testsample.n;i++) {
		...
		l=loss(testsample.examples[i].y,y,&sparm,testsample.examples[i].x.doc->fvec);
		avgloss+=l;
		if(l == 0) 
			correct++;
		else
						   ...
						   long *verbosity,long *struct_verbosity)
{
	...
	for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
		switch ((argv[i])[1]) 
		{ 
		case 'h': print_help_c(); exit(0);
		case '?': print_help_c(); exit(0);
